...
class PluginReloadHandler(FileSystemEventHandler):
    def on_modified(self,event):
        if event.src_path.endswith(".py"): asyncio.create_task(reload_plugin(event.src_path))
...
async def reload_plugin(path):
    ...
    import_path = f"plugins.{plugin_name}"
    try:
        if import_path in sys.modules: del sys.modules[import_path]; LOADED_PLUGINS.discard(plugin_name)
        await import_plugin(path, plugin_name)
        print(f"🔄 Plugin reloaded => {plugin_name}")
        if dreamxbotz.is_connected:
            await dreamxbotz.send_message(LOG_CHANNEL,f"🔄 **Plugin reloaded:** `{plugin_name}`")
    except Exception as e:
        print(f"⚠ Failed to reload plugin {plugin_name}: {e}")

def start_plugin_watcher():
    observer = Observer()
    observer.schedule(PluginReloadHandler(), str(PLUGINS_FOLDER), recursive=False)
    observer.start()
    return observer
...
async def show_plugins_auto(client:Client,message:Message):
    ...
    except Exception as e: await message.reply_text(f"⚠ Dashboard stopped:\n{e}")
...
async def reload_plugin_command(client:Client,message:Message):
    try:
        ...
        await reload_plugin(plugin_path)
...
async def dreamxbotz_start():
    ...
    observer=start_plugin_watcher(); dreamxbotz.loop.create_task(asyncio.to_thread(observer.join))
