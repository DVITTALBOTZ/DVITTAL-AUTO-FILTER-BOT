    ...
    try:
        ...
        if dreamxbotz.is_connected:
            await dreamxbotz.send_message(
                LOG_CHANNEL, f"✅ **Plugin loaded:** `{plugin_name}`"
            )
        ...
        if dreamxbotz.is_connected:
            await dreamxbotz.send_message(
                LOG_CHANNEL, f"⚠ **Plugin failed:** `{plugin_name}`\n`{e}`"
            )
...
async def reload_plugin(path):
    ...
    try:
        if import_path in sys.modules:
            ...
            LOADED_PLUGINS.discard(plugin_name)
        await import_plugin(path, plugin_name)
        print(f"🔄 Plugin reloaded => {plugin_name}")
        if dreamxbotz.is_connected:
            await dreamxbotz.send_message(
                LOG_CHANNEL, f"🔄 **Plugin reloaded:** `{plugin_name}`"
            )
    except Exception as e:
...
async def show_plugins_auto(client: Client, message: Message):
    try:
        ...
        await message.reply_text(f"⚠ Dashboard stopped:\n{e}")
...
async def reload_plugin_command(client: Client, message: Message):
    try:
        if len(message.command) < 2:
            await message.reply_text("⚠ Usage: /reload <plugin_name>")
            return
        plugin_name = message.command[1].strip()
        plugin_path = f"plugins/{plugin_name}.py"
        if not Path(plugin_path).exists():
            await message.reply_text(f"❌ Plugin `{plugin_name}` does not exist.")
            return
